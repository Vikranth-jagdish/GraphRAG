    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        if self.config.extract_entities:
            ...
            entities_extracted = sum(
                ...
                len(chunk.metadata.get("entities", {}).get("tests", [])) +
                len(chunk.metadata.get("entities", {}).get("procedures", [])) +
                len(chunk.metadata.get("entities", {}).get("symptoms", [])) +
                len(chunk.metadata.get("entities", {}).get("values", [])) +
                len(chunk.metadata.get("entities", {}).get("organizations", []))
                for chunk in chunks
            )
