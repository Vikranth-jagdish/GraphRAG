...
try:
    ...
    # Fallback: Use OpenAI-compatible interface for Gemini
    try:
...
def get_llm_model(model_choice: Optional[str] = None) -> Union[OpenAIModel, 'GeminiModel']:
    ...
    if llm_provider == 'gemini':
        if GEMINI_AVAILABLE:
            # Use native Pydantic AI Gemini support if available
            provider = GeminiProvider(api_key=api_key)
            return GeminiModel(llm_choice, provider=provider)
        elif GOOGLE_GENAI_AVAILABLE:
            # Configure Google GenAI client
            genai.configure(api_key=api_key)
            # Fall back to OpenAI-compatible interface for Gemini
            provider = OpenAIProvider(base_url=base_url, api_key=api_key)
            return OpenAIModel(llm_choice, provider=provider)
        else:
            raise ImportError("Gemini support requires 'google-generativeai' package")
