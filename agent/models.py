...
class DiagnosisItem(BaseModel):
    """Differential diagnosis item with citations."""
    tag: str = Field(..., description="Condition identifier in snake_case")
    title: str = Field(..., description="Full condition name")
    details: str = Field(..., description="Clinical rationale")
    citations: List[Citation] = Field(default_factory=list, description="Supporting evidence")


class AssessmentPlanItem(BaseModel):
    """Assessment plan item with citations."""
    tag: Literal["labs", "imaging", "screening", "monitoring", "other"] = Field(..., description="Assessment category")
    title: str = Field(..., description="Investigation/test name")
    details: str = Field(..., description="Detailed rationale")
    citations: List[Citation] = Field(default_factory=list, description="Supporting evidence")


class TreatmentRecommendationItem(BaseModel):
    """Treatment recommendation with citations."""
    tag: Literal["patient_education", "pharmacologic", "non_pharmacologic", "follow_up", "emergency"] = Field(..., description="Treatment category")
    title: str = Field(..., description="Treatment/intervention name")
...
class DifferentialDiagnosisRequest(BaseModel):
    ...
    clinical_information: str = Field(..., description="Patient clinical presentation")
    patient_id: str = Field(..., description="Patient identifier")
    patient_context: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional patient context")


class DifferentialDiagnosisResponse(BaseModel):
    """Response with differential diagnoses."""
    success: bool = True
    differential_diagnoses: List[DiagnosisItem] = Field(default_factory=list)
...
class TreatmentPlanRequest(BaseModel):
    ...
    clinical_information: str = Field(..., description="Patient clinical presentation")
    selected_diagnoses: List[DiagnosisItem] = Field(..., description="Selected differential diagnoses")
    patient_id: str = Field(..., description="Patient identifier")
    patient_gender: Optional[str] = Field(None, description="Patient gender for appropriate recommendations")
    patient_context: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Additional patient context")


class TreatmentPlanResponse(BaseModel):
    """Response with assessment plan and treatment recommendations."""
    success: bool = True
    assessment_plan: List[AssessmentPlanItem] = Field(default_factory=list)
    treatment_recommendations: List[TreatmentRecommendationItem] = Field(default_factory=list)
    generated_at: datetime = Field(default_factory=datetime.now)
    patient_id: str